...
export async function initializeFirestore(databaseId?: string): Promise<Firestore> {
  try {
    console.log(`Initializing Firestore with database: ${databaseId || '(default)'}`);
    console.log(`Project ID: ${default_config!.projectId || serviceAccount.project_id}`);
