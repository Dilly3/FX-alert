...
export class FirestoreUserStore implements UserDataStore {
  ...
  async firstOrCreate(user: UserInfo): Promise<UserInfo> {
    try {
      ...
      const pinExpiryTime =
        user.pinExpiryTime instanceof Date
  ...
  async getVerificationPin(email: string): Promise<string> {
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      throw new Error(
        `Failed to get verification pin for user ${email}: ${message}`
      );
    }
  }
  ...
  private async getVerifiedUsersPaginated(
    pageSize: number,
    lastDoc?: number
  ): Promise<{ users: UserInfo[]; lastDoc: any }> {
    try {
      let query = this.db
        .collection(this.COLLECTION_NAME)
      ...
      const users: UserInfo[] = [];
      let lastDocument: QueryDocumentSnapshot<FirebaseFirestore.DocumentData> | null =
        null;
  ...
  async getAllVerifiedUsers(): Promise<UserInfo[]> {
    ...
    while (hasMore) {
      try {
        ...
        console.log(
          `Fetched ${result.users.length} verified users, total: ${allUsers.length}`
        );
      } catch (error) {
  ...
  validatePin(pin: string, user: UserInfo): boolean {
    ...
    const expiryTime =
      typeof user.pinExpiryTime === "string"
