...
export class PgCurrencyStore implements CurrencyDataStore {
  ...
  async firstOrCreate(currency: Currency): Promise<Currency> {
    try {
      ...
      if (!result) {
        throw new Error(
          `Failed to create or find currency with code: ${currency.code}`
        );
      }
  ...
  async getCurrency(code: string): Promise<Currency | null> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(
          `Failed to get currency with code ${code}: ${error.message}`
        );
      }
  ...
  async updateCurrency(currency: Currency): Promise<Currency> {
    try {
      ...
      if (!updatedCurrency) {
        throw new Error(
          `Failed to retrieve updated currency with code: ${currency.code}`
        );
      }
  ...
  async deleteCurrency(code: string): Promise<void> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(
          `Failed to delete currency with code ${code}: ${error.message}`
        );
      }
