...
export class RedisClient {
  ...
  constructor(
    ...
    const { client, CACHE_TTL, isUp } = this.connect(
      username,
      password,
      host,
      port,
  ...
  private connect(
    ...
    port: number,
    ttl: number
  ): { client: RedisClientType; CACHE_TTL: number; isUp: boolean } {
  ...
  async getCurrencyRate(
    request: LiveRatesRequest
  ): Promise<LiveRatesResponse | null> {
    const cacheKey = Array.isArray(request.currencies)
      ? `currency_rate_${request.base}_${request.currencies.sort().join(",")}`
      : `currency_rate_${request.base}_${request.currencies}`;
